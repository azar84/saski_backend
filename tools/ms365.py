...
def is_within_time_slots(input_time, input_timezone, time_slots):
  ...
  for slot in time_slots:
    # Parse slot start and end times as naive datetime objects
    start_dt_naive = datetime.fromisoformat(slot['startDateTime']['dateTime'])
    end_dt_naive = datetime.fromisoformat(slot['endDateTime']['dateTime'])

    # Localize these datetimes to UTC

    # Convert the slot times from UTC to the input timezone
    start_dt_local = start_dt_naive.astimezone(input_tz)
